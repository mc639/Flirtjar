<?php
	...
	$from = $_POST['from'];
	  ...
	  else
	  {
		...
		$FirstName = addslashes($_POST["HRFirstName"]);
		$LastName = addslashes($_POST["HRLastName"]);
		$ContactNo = $_POST["HRContactNo"];
		$Email = $_POST["HREmail"];
		$UserImage = $_FILES['UserImage']['name'];
		$Password = $_POST["HRPassword1"];
		$DisplayOrder = $_POST['DisplayOrder'];
		$Status = $_POST['Status'];
		
		$SelectHeaderEmail = $obj->sql_query("SELECT Email FROM tbl_register WHERE Email = '".$Email."'");
		if(count($SelectHeaderEmail) == 0)
		{
